...
const CliCmdType CMD_WRITE =
{
	...
	"\tUsage:       8relay <id> write <channel> <on/off>\n",
	"\tUsage:       8relay <id> write <value>\n",
	"\tExample:     8relay 0 write 2 On; Set Relay #2 on Board #0 On\n"};
...
const CliCmdType CMD_READ =
{
	...
	"\tUsage:       8relay <id> read <channel>\n",
	"\tUsage:       8relay <id> read\n",
	"\tExample:     8relay 0 read 2; Read Status of Relay #2 on Board #0\n"};
...
char *usage = "Usage:	 8relay -h <command>\n"
	"         8relay -v\n"
	"         8relay -warranty\n"
	"         8relay -list\n"
	"         8relay <id> write <channel> <on/off>\n"
	"         8relay <id> write <value>\n"
	"         8relay <id> read <channel>\n"
	"         8relay <id> read\n"
	"         8relay <id> test\n"
	"Where: <id> = Board level id = 0..7\n"
	"Type 8relay -h <command> for more help"; // No trailing newline needed here.
...
static void doRelayWrite(int argc, char *argv[])
{
	...
	if ( (argc != 5) && (argc != 4))
	{
		printf("Usage: 8relay <id> write <relay number> <on/off> \n");
		printf("Usage: 8relay <id> write <relay reg value> \n");
		exit(1);
	}
